...
3. 判断自己的 leaderTransferee 是否为空，如果不为空，则说明已经有 Leader Transfer 在执行。我们采用强制最新的原则，直接覆盖原来的 leaderTransferee，不管其是否执行成功；
...
和其他 cmd 的 propose 不一样，该指令是通过 d.RaftGroup.ProposeConfChange() 提交的，除此之外，其 propose 流程和其他 cmd 的大同小异。
